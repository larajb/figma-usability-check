    ...
    <div>
        ...
        <table v-show="showSmell" class="usability-smells-result__table">
            <tr class="type--pos-medium-normal" v-if="smell.steps.length === 1">
                ...
                <td v-if="type === 'scenario'" align="top">Transition {{ smell.steps[0] }}</td>
                <td v-else-if="type === 'task'" align="top">Step {{ smell.steps[0] }} ({{ getTimeFromTo(content.evaluationRuns[0].goms.stepsTimes, smell.steps[0]) }})</td>
            </tr>
            <tr class="type--pos-medium-normal" v-else-if="smell.steps.length > 1">
                <td valign="top">Gefunden in</td>
                <td v-if="type === 'scenario'" align="top">Transitions {{ getAsStringScenario(smell.steps[0]) }}</td>
                <td v-else-if="type === 'task'" align="top">Steps {{ getAsStringTask(content.evaluationRuns[0].goms.stepsTimes, smell.steps) }}</td>
            </tr>
